...
class myTableView(QTableView):
    def __init__(self, parent=None):
        ...
        iconFavorite = QIcon()
        iconFolder = QIcon()
        iconDelete.addPixmap(QPixmap(_fromUtf8(":/icons/delete_m.png")).copy(40, 0, 20, 20), QIcon.Normal, QIcon.Off)
...
class myFavoriteTable(QTableView):
    def __init__(self, parent=None):
        super(myFavoriteTable, self).__init__(parent)
        self.contextMenu = QMenu(self)
        iconDelete = QIcon()
        iconSave = QIcon()
        iconFavorite = QIcon()
...
class lyricTable(QLabel):
    ...
    def paintEvent(self, event):
        ...
        if textRectM.height() and self.intervel != 0:
            self.t.stop()
            self.t.start(float(self.intervel)/(textRectM.height() + self.adjust))
